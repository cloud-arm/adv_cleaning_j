...
if ($id == 0) {
    ...
    $sql = "INSERT INTO employee (name, type, phone_no, nic, address, attend_date, rate, des, des_id, epf_no, epf_amount, ot,rate_type,s_ot_rate, action, username, password, pic, position) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
    $q = $db->prepare($sql);
    $q->execute(array($name, $type, $phone_no, $nic, $address, $attend_date, $rate, $des, $des_id, $etf_no, $etf_amount, $ot,$sal_type, $well, 1, $user_name, $password, $imageUploadPath, $finalPosition));
    ...
    $sql = "UPDATE employee SET name = ?, address = ?, nic = ?, phone_no = ?,rate = ?, des = ?, des_id = ?, epf_amount = ?, epf_no = ?, ot = ?, s_ot_rate = ?,rate_type = ?, username = ?, password = ?, type = ?, pic = ? WHERE id = ? ";
    $q = $db->prepare($sql);
    $q->execute(array($name, $address, $nic, $phone_no, $rate, $des, $des_id, $etf_amount, $etf_no, $ot, $well, $sal_type,$user_name, $password, $type, $imageUploadPath, $id));
}
